import { Button, Box, Modal, ButtonGroup } from "@mui/material"
...
const BlogCard: React.FC<BlogCardProps> = ({blog, onUpdate, inprofile, inhomepage}) => {
    ...
    const {data: userVerification} = useQuery<boolean>({
    ...
    const {mutate: updateBlog} = useMutation({
      mutationFn: async (updatedBlog: UpdatedBlog) => {
        try {
    ...
    const [updatedBlog, setupdatedBlog] = useState<UpdatedBlog>(blog);
    
    const handleUpdateBlog = async (bid: string, updatedBlog: UpdatedBlog) => {
      ...
      const updatedBlogData = {
        ...updatedBlog,
        content: contentHtml,
        image: updatedBlog.imageurl || updatedBlog.image,
      };
    ...
    const [previewModalBodyOpen, setpreviewModalBodyOpen] = useState<boolean>(false);
    const [previewThumbnailOpen, setpreviewThumbnailOpen] = useState<boolean>(false);
    const [fullversion, setfullversion] = useState<boolean>(true);
    ...
    const [freshBlog, setfreshBlog] = useState<UpdatedBlog>({
      title: "",
      content: "",
      image: "",
      ...
      <>
          <Box className="w-[22rem] min-h-[14rem] rounded-sm border border-gray-500 transition-transform transform hover:-translate-y-1.5">
            <button className="w-full h-[12rem] border-b border-gray-500">
              ...
              <div className="w-full h-[12rem] border-b border-gray-500 relative">
                <Image
                  src={blog.image}
                  alt="Blog image"
                  fill
          ...
          <div className='w-full min-h-screen bg-gray-100 dark:bg-gray-900'>
            ...
            <div className="relative w-[60%] mx-auto bg-gray-100 dark:bg-gray-900">
              <div className="relative h-fit-content bg-cover bg-center">
                ...
                <div className="relative w-full h-[24rem] mt-5">
                  <Image 
